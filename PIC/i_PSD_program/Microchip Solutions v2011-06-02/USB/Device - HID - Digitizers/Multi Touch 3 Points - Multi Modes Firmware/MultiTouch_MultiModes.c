/********************************************************************
 ...
 Hardware:		This demo is natively intended to be used on Microchip USB demo
 				...
 				platforms.
 Complier:  	Microchip C18 (for PIC18), C30 (for PIC24/dsPIC33), C32 (for PIC32)
...
void UserGetReportHandler(void)
{
	...
	if(SetupPkt.wValue == (0x0300 + VALID_CONTACTS_FEATURE_REPORT_ID))	
	{
		...
		//send the packet
		inPipes[0].pSrc.bRam = (BYTE*)&FeatureReport;
	...
	else if(SetupPkt.wValue == (0x0300 + DEVICE_MODE_FEATURE_REPORT_ID))	//Report ID = 0x03 = Configuration report, which is used to select between mouse, single/multi digitizer modes.
	{
		...
		{
			//Size of the feature report.  Byte 0 is report ID, byte 1 is the current DeviceMode, Byte 2 is
			//the DeviceIdenfier
			inPipes[0].wCount.Val = 3;		
		}		
		inPipes[0].info.bits.ctrl_trf_mem = USB_EP0_RAM;
		inPipes[0].info.bits.busy = 1;			
	}		
