...
#if defined (__18F14K50)|| defined(__18LF14K50)
    #include "audio2.h"
#else
    #include "audio1.h"
#endif
...
/******************************************************************************
 * Function:        void EmulateMicrophone(void)
 *
 * PreCondition:    None
 *
 * Input:           None
 ...
 *
 * Overview:        This routine will emulate the function of the microphone.  It
 *					does this by sending IN packets of data to the host.
 *					It sends the audio data stored in the audio1.h file to host.
 *					The function emulates a PCM, 8Khz, 16bits/Second, Mono Microphone.
 *
 *
 * Note:
 *
 *****************************************************************************/
 void EmulateMicrophone(void)
 {
     ...
     if (transferUnderProgress == 0)
     {
         if(sw2==0)
    	 {
        	 // if the pushbutton Sw2 is pressed and there is no transfer currently in progress then
        	 // mark the flag to start sending the audio data from the file to the host.
       	     transferUnderProgress = 1;
    	 }
	 }
     if(!USBHandleBusy(USBTxHandle))
     {
         if(transferUnderProgress == 1)
		 {
			 ...
			 if (FrameCounter >= sizeof(audioSamples))
			 {
			     FrameCounter = 0;
				 transferUnderProgress = 0;
			 }
			 USBTxHandle = USBTxOnePacket(AS_EP , FrameData,NO_OF_BYTES_TRANSFRED_IN_A_USB_FRAME);
		  }
		  else
		  {
			 for(sampleCounter =0; sampleCounter < NO_OF_BYTES_TRANSFRED_IN_A_USB_FRAME; sampleCounter++)
			 {
				FrameData[sampleCounter] =   0x00;
			 }
			 USBTxHandle = USBTxOnePacket(AS_EP , FrameData,NO_OF_BYTES_TRANSFRED_IN_A_USB_FRAME);
		  }
     }
