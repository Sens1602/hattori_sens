...
//（おそらく）振動子12MHzから3分周で4MHzを生成→そっからPLLで96MHzを生成してまた2分周して48MHzを生成、USBクロックとシステムクロックに使用、という構成：データシート2550,TABLE 2-3:
...
//	WPUBbits.WPUB4 = 1;			// RB4 pull up
    ...
    //debug_i(1,2,3);
    rs1 = RI2C(0b11010000, 0x75);
    //debug_i(rs1, 1, 2);   
    
    err = SendI2C(0b11010000, 0x6B, 0x00);
    rs2 = RI2C(0b11010000, 0x6B);
  
    err = SendI2C(0b11010000, 0x37, 0x02);
...
unsigned int SendI2C( unsigned char control, 
