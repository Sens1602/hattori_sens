...
void USB_CDC_Process(){
    ...
    if(state == 0){
        ...
        ph = (char*)thigh;        
        memcpy(ph, &USB_In_Buf[1], 1);
        ph++;
        memcpy(ph, &USB_In_Buf[2], 1);
        ph++;
        memcpy(ph, &USB_In_Buf[3], 1);
        ph++;
        memcpy(ph, &USB_In_Buf[4], 1);
        ph++;        
        state++;
        ...
        if(USBUSARTIsTxTrfReady()){
            putUSBUSART(USB_In_Buf + sizeof(unsigned char), 9);
            CDCTxService();
            cycready = 1;
            WriteTimer0(0);
    ...
    PORTCbits.RC6 = 1;
    for(counter3=0; counter3<thigh; counter3++){ 
        Nop();
    }
    PORTCbits.RC6 = 0;
